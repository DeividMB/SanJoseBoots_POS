...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 14-03-2026 a las 00:25:27
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_CerrarCaja` (IN `p_CajaID` INT, IN `p_UsuarioID` INT, IN `p_Param3` INT, IN `p_MontoFinalDeclarado` DECIMAL(10,2), IN `p_Notas` TEXT)   BEGIN
    DECLARE v_TotalVentas  DECIMAL(10,2) DEFAULT 0;
    DECLARE v_MontoInicial DECIMAL(10,2) DEFAULT 0;

    SELECT COALESCE(TotalVentas, 0), MontoInicial
    INTO   v_TotalVentas, v_MontoInicial
    FROM   Cajas
    ...
    ELSE
        UPDATE Cajas SET
            ...
            MontoFinalReal      = v_MontoInicial + v_TotalVentas,
            Diferencia          = p_MontoFinalDeclarado - (v_MontoInicial + v_TotalVentas),
            NotasCierre         = p_Notas,
            Estado              = 'Cerrada'
        WHERE CajaID = p_CajaID;
        ...
        SELECT 'Caja cerrada exitosamente'                              AS Mensaje,
               'SUCCESS'                                                AS Estado,
               v_MontoInicial + v_TotalVentas                          AS MontoFinalReal,
               p_MontoFinalDeclarado - (v_MontoInicial + v_TotalVentas) AS Diferencia;
    END IF;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_HistorialCajas` (IN `p_FechaInicio` DATE, IN `p_FechaFin` DATE, IN `p_UsuarioID` INT)   BEGIN
...
WHERE DATE(c.FechaHoraApertura) 
...
(6, 1, '2026-03-13 17:06:53', NULL, 500.00, NULL, NULL, 1740.00, 986.00, 1160.00, 3886.00, 3, NULL, NULL, NULL, 'Abierta');
...
(1, 1, 'BOT-001-25-NEG', '25', 'Negro', 'Clasico', 1800.00, 1, 2, 1, '2026-02-06 23:01:31'),
...
(21, 8, 'BOT-002-27-CAF', '27', 'Cafe', 'Trabajo', 1500.00, 6, 3, 1, '2026-02-06 23:01:31'),
...
(27, 6, 'ACC-001-U-NEG', 'Unica', 'Negro', 'Clasico', 280.00, 22, 8, 1, '2026-02-10 02:15:37'),
...
(1, 'Administrador del Sistema', 'admin@sanjoseboots.com', 'admin', '$2a$10$5ULwTtzJegPldP/yFLBSkOT1v7omEKIz5lvNySZlSoZxt.2/4pnly', 1, 1, '2026-02-06 23:01:30', '2026-03-13 17:02:02'),
...
--
ALTER TABLE `cajas`
  MODIFY `CajaID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=7;
...
--
ALTER TABLE `detalleventas`
  MODIFY `DetalleVentaID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=30;
